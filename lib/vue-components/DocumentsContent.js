...
**Note: This API documentation is still in draft status.**`,
        ...
        async downloadMarkdown(documentName) {
            try {
                ...
                // Fetch the document configuration to get the markdown files
                const configResponse = await fetch(`/api/documents/${documentName}/config`)
                if (!configResponse.ok) {
                    throw new Error('Failed to fetch document configuration')
                }
                
                const config = await configResponse.json()
                const markdownFiles = config.markdown_files || []
                
                if (markdownFiles.length === 0) {
                    throw new Error('No markdown files found in document configuration')
                }
                
                // Fetch and combine all markdown files
                let combinedMarkdown = `# ${config.document_title || documentName}\n\n`
                combinedMarkdown += `*Version: ${config.version || '1.0'}*  \n`
                combinedMarkdown += `*Date: ${config.date || 'Unknown'}*\n\n`
                combinedMarkdown += '---\n\n'
                
                for (const file of markdownFiles) {
                    try {
                        const fileResponse = await fetch(`/api/files/${file}`)
                        if (fileResponse.ok) {
                            const fileData = await fileResponse.json()
                            combinedMarkdown += `${fileData.content}\n\n`
                        } else {
                            combinedMarkdown += `*[File ${file} could not be loaded]*\n\n`
                        }
                    } catch (fileError) {
                        console.error(`Error loading ${file}:`, fileError)
                        combinedMarkdown += `*[Error loading ${file}]*\n\n`
                    }
                }
