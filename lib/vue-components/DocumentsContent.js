...
const DocumentsContent = {
    ...
    methods: {
        async loadDocuments() {
            ...
            try {
                ...
                try {
                    ...
                    console.log('API not available, scanning documents directory directly...')
                    
                    // Fallback: scan documents directory by trying known/common document names
                    ...
                    for (const docName of commonDocuments) {
                        try {
                            const configResponse = await fetch(`documents/${docName}/config.yaml`)
                ...
                this.availableDocuments = documentsFound.map(doc => ({
                    name: doc.name,
                    title: doc.title,
                    lastUpdated: this.formatDate(doc.lastUpdated),
                    status: doc.status || 'complete'
                }))
                
                // If no documents found, show a message
                if (this.availableDocuments.length === 0) {
                    this.showMessage('No documents found in the documents directory', 'info')
                }
        ...
        extractDocumentTitle(documentName, config) {
            // Try to get title from config, otherwise generate from document name
        ...
        async loadSelectedDocument() {
            ...
            try {
                ...
                if (result.config) {
                    this.selectedDocumentTitle = result.config.title || this.selectedDocumentTitle
